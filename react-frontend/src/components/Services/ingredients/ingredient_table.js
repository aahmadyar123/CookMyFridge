...
export default function IngredientTable(new_ingredient) {
  ...
  const [ingredients, setIngredients] = React.useState(value.ingredients);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = ingredients.map((n) => n);
  ...
  const visibleRows = React.useMemo(
    () =>
      stableSort(ingredients, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage,
      ),
    [order, orderBy, page, rowsPerPage, ingredients],
  );

  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar numSelected={selected.length} deleteIngredient={handleDelete}/>
        <TableContainer>
          <Table
            sx={{ minWidth: 750 }}
